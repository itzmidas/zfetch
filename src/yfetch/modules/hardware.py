...
from yfetch.utils.system import format_bytes, read_file_safe, run_command_safe
...
class GPUModule(BaseModule):
    ...
    def fetch(self) -> Optional[ModuleResult]:
        try:
            ...
            if not gpus:
                # 2. Fallback to sysfs DRM devices if lspci is not installed
                for uevent_path in sorted(glob.glob("/sys/class/drm/card*/device/uevent")):
                    uevent_content = read_file_safe(uevent_path)
                    if uevent_content:
                        for uline in uevent_content.splitlines():
                            if uline.startswith("DRIVER="):
                                driver = uline.split("=", 1)[1].strip()
                                if driver == "amdgpu":
                                    gpus.append("AMD Radeon Graphics")
                                elif driver in ("nvidia", "nouveau"):
                                    gpus.append("NVIDIA Graphics")
