...
def get_default_log_path() -> Path:
    ...
    config_dir = Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "yfetch"
    return config_dir / "debug.log"


def setup_logger(debug_mode: bool = False, log_file: Optional[Path] = None) -> logging.Logger:
    """Initialize and return the yfetch logger."""
    global _logger
    logger = logging.getLogger("yfetch")
