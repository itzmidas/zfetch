"""Interactive TUI configuration tool for zfetch built with Textual."""
...
from zfetch.themes.definitions import THEMES, list_themes
from zfetch.themes.quick_styles import QUICK_STYLES, apply_quick_style_by_name
from zfetch.tui.screens.save_modal import SaveExitModal
...
class ZFetchApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="app-container"):
            with Vertical(id="left-panel"):
                yield Label("⚡ zfetch Setup & Customization", id="panel-title")
    ...
    def show_main_menu(self) -> None:
        ...
        title_label.update("⚡ zfetch Setup & Customization")
    ...
    def show_advanced_menu(self) -> None:
        ...
        items = [
            MenuItem("back", "⬅ Back to Main Menu", "Return to main settings"),
            MenuItem("toggle_debug", f"Debug Mode: {'ON' if self.cfg.general.debug else 'OFF'}", "Log detailed traces to ~/.config/zfetch/debug.log"),
...
def run_tui(config_path: Optional[Path | str] = None) -> int:
    ...
    app = ZFetchApp(config_path=config_path)
    result = app.run()
