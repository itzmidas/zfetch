"""Configuration manager for loading, saving, and migrating zfetch config."""
...
def get_default_config_dir() -> Path:
    """Return ~/.config/zfetch or $XDG_CONFIG_HOME/zfetch."""
    base = os.environ.get("XDG_CONFIG_HOME")
    if base:
        return Path(base) / "zfetch"
    return Path.home() / ".config" / "zfetch"


def get_default_config_path() -> Path:
    """Return path to ~/.config/zfetch/config.toml."""
    return get_default_config_dir() / "config.toml"


def get_custom_ascii_dir() -> Path:
    """Return path to ~/.config/zfetch/ascii/."""
    return get_default_config_dir() / "ascii"


def ensure_config_dirs() -> None:
    """Ensure ~/.config/zfetch and ~/.config/zfetch/ascii exist."""
    try:
...
class ConfigManager:
    """Manages reading and writing zfetch configuration."""
